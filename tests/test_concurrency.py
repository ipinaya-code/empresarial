...
async def run_scenario(scenario_name, endpoint, num_requests=5):
...
async def main():
    ...
    await run_scenario("SIN CONTROL (Condición de Carrera)", "/reservar/inseguro", 5)

    # 2. Escenario Seguro (Con Bloqueo Pesimista)
    await run_scenario("CON BLOQUEO PESIMISTA (SELECT FOR UPDATE)", "/reservar/seguro", 5)
